...
pub async fn get_dental_clinic_by_id(
    ...
    let row = dental_clinic::Entity::find()
        ...
        .columns([
            ...
            // FIX: include acct_* columns because DentalClinicRowDb expects them
            dental_clinic::Column::AcctTin,
            dental_clinic::Column::AcctBankName,
            dental_clinic::Column::AcctAccountTypeId,
            dental_clinic::Column::AcctAccountName,
...
pub async fn create_dental_clinic(
    ...
    let am = dental_clinic::ActiveModel {
        ...
        acct_account_type_id: Set(body.acct_account_type_id),
        ...
        acct_tax_type_id: Set(body.acct_tax_type_id),
        acct_tax_classification_id: Set(body.acct_tax_classification_id),
...
pub async fn patch_dental_clinic(
    ...
    if let Some(v) = body.remarks {
        am.remarks = Set(v.map(|s| s.trim().to_string()).filter(|s| !s.is_empty()));
    }
    if let Some(v) = body.contact_numbers {
        am.contact_numbers = Set(v.map(|s| s.trim().to_string()).filter(|s| !s.is_empty()));
    }
    if let Some(v) = body.email {
        am.email = Set(v.map(|s| s.trim().to_string()).filter(|s| !s.is_empty()));
    }
    if let Some(v) = body.schedule {
        am.schedule = Set(v.map(|s| s.trim().to_string()).filter(|s| !s.is_empty()));
    }
